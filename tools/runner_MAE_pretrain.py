    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if dataset_name == 'ShapeNet':
                ...
                num_group = data[1]
                group_size = data[2]
                level = [id.split("@")[-1] for id in taxonomy_ids]
            else:
            ...
            _loss = base_model(points, num_group=num_group, group_size=group_size, level=level)
            ...
            if args.distributed:
                loss_1 = dist_utils.reduce_tensor(_loss, args)
                losses.update([loss_1.item()])
            else:
                losses.update([loss_1.item()])
