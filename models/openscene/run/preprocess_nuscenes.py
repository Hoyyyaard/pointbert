...
def get_parser():
    ...
    parser.add_argument('--config', type=str,
                    default='config/nuscenes/ours_openseg_pretrained.yaml',
                    help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    from dataset.nuscenes_loader import FusedFeatureLoader, collation_fn_eval_all
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse, locs_in, feat_in, instance_labels, scan_name) in enumerate(tqdm(val_data_loader)):
                ...
                # base_dir = '/home/admin/Projects/Point-BERT/data/SceneVerse/OpenScene_Nuscenes_Features'
                base_dir = '/gpfs/u/home/LMCG/LMCGljnn/scratch/zhy/pointbert/data/SceneVerse/OpenScene_Nuscenes_Features'
                ...
                if feature_type == 'distill':
                    ...
                    save_dict = {
                        'features': predictions.half().cpu().numpy(),
                        'points': locs_in.cpu().numpy(),
                        'colors': None,  # 如果 colors 是空的，可以直接设置为 None 或者其他默认值
                        'instance_labels': instance_labels.cpu().numpy(),
                        # 'inds_reverse': inds_reverse, 
                    }
                    np.savez('{}/{}.npz'.format(op_dir, scan_name), **save_dict)
